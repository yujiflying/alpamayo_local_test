...
def _resolve_vlm_path(vlm_name_or_path: str) -> str:
    """Resolve VLM repo/path with optional local override for offline inference."""
    override = os.environ.get("ALPAMAYO_VLM_PROCESSOR_PATH", vlm_name_or_path)
...
class ReasoningVLAConfig(PretrainedConfig):
    ...
    def _build_processor(self) -> AutoProcessor:
        ...
        expanded = _resolve_vlm_path(self.vlm_name_or_path)
        print(f"[base_model] loading VLM processor from: {expanded}")
        processor = AutoProcessor.from_pretrained(expanded, **processor_kwargs)
...
class ReasoningVLA(PreTrainedModel, TrajectoryFusionMixin):
    ...
    def _build_tokenizer(self, config: ReasoningVLAConfig) -> Any:
        ...
        expanded = _resolve_vlm_path(config.vlm_name_or_path)
        print(f"[base_model] loading tokenizer processor from: {expanded}")
        processor = AutoProcessor.from_pretrained(expanded, **processor_kwargs)
    ...
    def _initialize_qwenvl3_vlm(self, config: ReasoningVLAConfig) -> None:
        ...
        """
        expanded = _resolve_vlm_path(config.vlm_name_or_path)
        print(f"[base_model] loading VLM config from: {expanded}")
    ...
    def from_pretrained_submodules(
        ...
        # Load VLM
        expanded = _resolve_vlm_path(config.vlm_name_or_path)
        print(f"[base_model] loading pretrained VLM from: {expanded}")
