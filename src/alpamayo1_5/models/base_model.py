...
class ReasoningVLAConfig(PretrainedConfig):
    ...
    def _build_processor(self) -> AutoProcessor:
        ...
        vlm_processor_path = os.environ.get("ALPAMAYO_VLM_PROCESSOR_PATH", self.vlm_name_or_path)

        if isinstance(vlm_processor_path, str):
            expanded = str(Path(vlm_processor_path).expanduser())
        else:
            expanded = vlm_processor_path
...
class ReasoningVLA(PreTrainedModel, TrajectoryFusionMixin):
    ...
    def _build_tokenizer(self, config: ReasoningVLAConfig) -> Any:
        ...
        vlm_processor_path = os.environ.get("ALPAMAYO_VLM_PROCESSOR_PATH", config.vlm_name_or_path)
