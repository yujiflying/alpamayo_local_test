...
def main() -> None:
    ...
    if not model_path.exists():
        raise FileNotFoundError(
            f"Model path does not exist in current runtime: {model_path}. "
            "If you are running inside a container, pass the in-container path with --model-path."
        )

    #model = Alpamayo1_5.from_pretrained(args.model_path, dtype=torch.bfloat16).to(args.device)
    model = Alpamayo1_5.from_pretrained(str(model_path), dtype=torch.bfloat16).to(args.device)
