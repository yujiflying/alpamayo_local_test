...
def _load_ego_pose_log(
    ...
    Uses only $GPFPD rows and ignores all other rows such as $GTIMU.

    Confirmed $GPFPD field semantics:
        0:  $GPFPD
        1:  GPS week
        2:  GPS time (seconds-of-day)
        3:  Heading (deg)          [ENU yaw convention: east=0, CCW positive]
        4:  Pitch (deg)
        5:  Roll (deg)
        6:  Latitude (deg)
        7:  Longitude (deg)
    ...
    Returns:
        timestamps_sod: [N]
        xyz_enu: [N, 3] in meters, converted from lat/lon/alt into local ENU
        rotations: scipy Rotation of length N
    """
    ...
    with ego_log_path.open("r") as f:
        for line_idx, raw_line in enumerate(f, start=1):
            ...
            try:
                gps_week = float(parts[1])   # parsed for completeness
                sod = float(parts[2])
                ...
                alt_m = float(parts[8])
                ve_mps = float(parts[9])     # parsed for optional debug / validation
                vn_mps = float(parts[10])    # parsed for optional debug / validation
                vu_mps = float(parts[11])    # parsed for optional debug / validation
    ...
    # Heading is already confirmed to be ENU yaw:
...
def load_offline_dataset(
    ...
    # - Apply the fixed transform to rotations so the final local representation
    #   matches the model's +x-forward yaw convention downstream.
    R_fix = np.array(
        [
            [0.0, -1.0, 0.0],
            [1.0,  0.0, 0.0],
