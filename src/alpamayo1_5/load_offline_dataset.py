...
IMPORTANT:
- This version intentionally does NOT cache fully decoded video frames.
- Full-video RGB frame caching can easily exhaust notebook memory and kill the kernel.
- We only cache pose parsing results and camera file discovery.
- Single-window and sliding-window notebooks should share evaluation logic via
  ``offline_eval_utils.py``.
"""
...
def _load_ego_pose_log(
    ...
    Returns:
        ...
        rotations_global_enu:
            scipy Rotation object of length N.
    """
...
def _load_video_frames(video_path: str | Path) -> list[np.ndarray]:
    ...
    NOTE:
    - This function decodes a video on demand.
    - We intentionally do NOT keep the decoded frame list in a long-lived cache.
    """
...
def load_offline_dataset(
    ...
    Public return fields used by the existing notebooks remain compatible:
        ...
        - ego_future_rot
        - relative_timestamps
        - absolute_timestamps_sod
        - video_frame_indices
        - actual_video_frame_indices
        - video_num_frames
        - frame0_gps_time_sod
        - fps
        - clip_dir

    New structured diagnostics are also returned to help notebook-based debugging.

    Implementation detail:
    - pose data is cached
    - video frames are decoded on demand per call (not cached)
    """
