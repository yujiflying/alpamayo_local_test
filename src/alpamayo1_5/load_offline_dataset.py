...
def load_offline_dataset(
    ...
    # offline local-frame convention with the model/action-space convention.
    #
    # For xyz, this is a coordinate-frame rotation in the local XY plane:
    #   x' = -y
    #   y' =  x
    #
    # For rotations, the model's yaw extractor so3_to_yaw_torch() interprets
    # column 0 as the forward axis. Our offline rotations after Step 1 behave
    # like column 1 is forward, so we right-multiply by the same matrix as a
    # basis remapping to make col0=forward for the model.
    R_fix = np.array(
        [
            [0.0, -1.0, 0.0],
            [1.0,  0.0, 0.0],
    ...
    ego_history_rot_local = ego_history_rot_local @ R_fix[None, :, :]
    ego_future_rot_local = ego_future_rot_local @ R_fix[None, :, :]
