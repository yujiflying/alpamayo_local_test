...
3. Lightweight clip-level pose caching
4. Optional pre-extracted image-sequence backend for faster repeated evaluation
5. Partial-future support for very late t0 inference windows

IMPORTANT:
- This version does NOT cache fully decoded video frames in memory.
- It supports pre-extracted jpg frame folders under:
      clip_dir / "extracted_frames" / <video_stem> / 000000.jpg
- If extracted frames are not found, it automatically falls back to mp4 decoding.
- History must stay fully inside the ego pose log range.
- Future may partially exceed the ego pose log range; valid future steps are
  returned together with a validity mask.
"""
...
DEFAULT_EXTRACTED_FRAMES_ROOT_NAME = "extracted_frames"
...
def _load_ego_pose_log(
    ...
    """Load ego poses from ego_pos.log using $GPFPD rows only."""
    ...
    xyz_global_enu = _geodetic_to_local_enu(lat_deg=lat_deg, lon_deg=lon_deg, alt_m=alt_m)
...
def _get_extracted_frames_dir_for_video(
    video_path: Path,
    extracted_frames_root_name: str = DEFAULT_EXTRACTED_FRAMES_ROOT_NAME,
) -> Path:
    """Return expected extracted-frame directory for a given mp4 path."""
    return video_path.parent / extracted_frames_root_name / video_path.stem


def _try_get_num_extracted_frames(
    extracted_frames_dir: Path,
) -> int | None:
    """Return number of extracted jpg files if directory exists, otherwise None."""
    if not extracted_frames_dir.exists() or not extracted_frames_dir.is_dir():
        return None
    frame_files = sorted(extracted_frames_dir.glob("*.jpg"))
    if not frame_files:
        return None
    return len(frame_files)


def _sample_extracted_image_frames(
    extracted_frames_dir: Path,
    frame_indices: np.ndarray,
) -> tuple[torch.Tensor, np.ndarray, int]:
    """Sample frames from a pre-extracted jpg image sequence."""
    frame_files = sorted(extracted_frames_dir.glob("*.jpg"))
    if not frame_files:
        raise ValueError(f"No extracted jpg frames found under {extracted_frames_dir}")
    ...
    for index in clipped_indices:
        frame_path = extracted_frames_dir / f"{int(index):06d}.jpg"
        if not frame_path.exists():
            raise FileNotFoundError(
                f"Missing extracted frame file: {frame_path}. "
                f"Expected contiguous 6-digit jpg numbering."
            )
        image = Image.open(frame_path).convert("RGB")
        sampled.append(np.asarray(image, dtype=np.uint8))
...
def _sample_camera_frames_with_fallback(
    camera_path: Path,
    frame_indices: np.ndarray,
    *,
    debug: bool = False,
    ...
    """Prefer extracted jpg frames; fallback to mp4 decode if not available."""
    extracted_frames_dir = _get_extracted_frames_dir_for_video(
        camera_path,
        extracted_frames_root_name=extracted_frames_root_name,
    )
    num_extracted = _try_get_num_extracted_frames(extracted_frames_dir)

    if num_extracted is not None:
        _debug_print(
            debug,
            f"Using extracted frames for {camera_path.name}: {extracted_frames_dir} "
            f"(num_frames={num_extracted})",
        )
        return _sample_extracted_image_frames(extracted_frames_dir, frame_indices)

    _debug_print(debug, f"Falling back to mp4 decode for {camera_path.name}")
    return _sample_video_frames(camera_path, frame_indices)
...
def _build_time_alignment_summary(
    *,
    ...
    num_valid_future_steps = int(future_valid_mask.sum())
    num_invalid_future_steps = int((~future_valid_mask).sum())

    if num_valid_future_steps > 0:
        valid_future_times = future_times_sod[future_valid_mask]
        future_valid_time_range_sod = (
            float(valid_future_times[0]),
            float(valid_future_times[-1]),
        )
    else:
        future_valid_time_range_sod = None
    ...
    summary = {
        ...
        "future_valid_time_range_sod": future_valid_time_range_sod,
        ...
        "pose_margin_right_sec_using_full_future": float(pose_times_sod[-1] - future_times_sod[-1]),
        "num_valid_future_steps": num_valid_future_steps,
        "num_invalid_future_steps": num_invalid_future_steps,
        "future_truncated": bool(num_invalid_future_steps > 0),
    }
    return summary
...
def _debug_print_time_alignment_summary(debug: bool, summary: dict[str, Any]) -> None:
    ...
    print(f"  future_valid_time_range_sod: {summary['future_valid_time_range_sod']}")
    print(
        "  image_time_range_sod: "
    ...
    print(f"  pose_margin_right_sec_using_full_future: {summary['pose_margin_right_sec_using_full_future']:.6f}")
    print(f"  num_valid_future_steps: {summary['num_valid_future_steps']}")
    print(f"  num_invalid_future_steps: {summary['num_invalid_future_steps']}")
    print(f"  future_truncated: {summary['future_truncated']}")
...
class OfflineClipCache:
    ...
    def build(cls, clip_dir: str | Path, debug: bool = False) -> "OfflineClipCache":
        ...
        _debug_print(debug, f"Building pose cache for clip_dir={clip_dir}")
...
def get_or_build_offline_clip_cache(
    ...
    *,
    ...
    clip_dir_key = str(Path(clip_dir).expanduser().resolve())
    if force_rebuild or clip_dir_key not in _CLIP_CACHE_REGISTRY:
        _CLIP_CACHE_REGISTRY[clip_dir_key] = OfflineClipCache.build(clip_dir, debug=debug)
    return _CLIP_CACHE_REGISTRY[clip_dir_key]
...
def clear_offline_clip_cache(clip_dir: str | Path | None = None) -> None:
    ...
    clip_dir_key = str(Path(clip_dir).expanduser().resolve())
    _CLIP_CACHE_REGISTRY.pop(clip_dir_key, None)
...
def load_offline_dataset(
    ...
    Public return fields used by existing notebooks remain compatible.

    New behavior:
    - history must be fully valid
    - future may be partially valid
    - returns future validity mask + future truncation diagnostics
    """
    ...
    camera_paths = cache.camera_paths
    ...
    # History must be fully valid.
    if history_times_sod.min() < pose_times_sod[0] or history_times_sod.max() > pose_times_sod[-1]:
        raise ValueError(
            "Requested history window exceeds ego pose log range. "
            f"Pose range: [{pose_times_sod[0]:.3f}, {pose_times_sod[-1]:.3f}], "
            f"history requested: [{history_times_sod.min():.3f}, {history_times_sod.max():.3f}]"
        )

    future_valid_mask = (
        (future_times_sod >= pose_times_sod[0]) &
        (future_times_sod <= pose_times_sod[-1])
    )
    num_valid_future_steps = int(future_valid_mask.sum())
    future_truncated = bool(num_valid_future_steps < num_future_steps)
    ...
    # Build future arrays with fixed length, using NaNs for invalid future xyz.
    ego_future_xyz_global_enu = np.full((num_future_steps, 3), np.nan, dtype=np.float64)
    ego_future_rot_global_enu = np.tile(np.eye(3, dtype=np.float64)[None, :, :], (num_future_steps, 1, 1))

    if num_valid_future_steps > 0:
        valid_future_times_sod = future_times_sod[future_valid_mask]
        valid_future_xyz_global_enu, valid_future_rot_global_enu = _lookup_pose_samples(
            pose_times_sod,
            pose_xyz_global_enu,
            pose_rot_global_enu,
            valid_future_times_sod,
        )
        ego_future_xyz_global_enu[future_valid_mask] = valid_future_xyz_global_enu
        ego_future_rot_global_enu[future_valid_mask] = valid_future_rot_global_enu
    ...
    # Future xyz in t0-local: valid rows transformed, invalid rows kept as NaN.
    ego_future_xyz_t0_local = np.full((num_future_steps, 3), np.nan, dtype=np.float64)
    ego_future_rot_t0_local = np.tile(np.eye(3, dtype=np.float64)[None, :, :], (num_future_steps, 1, 1))
    if num_valid_future_steps > 0:
        valid_future_xyz_t0_local, valid_future_rot_t0_local = _convert_global_enu_pose_to_t0_local_frame(
            xyz_global_enu=ego_future_xyz_global_enu[future_valid_mask],
            rot_global_enu_mats=ego_future_rot_global_enu[future_valid_mask],
            t0_xyz_global_enu=t0_xyz_global_enu,
            t0_rot_global_enu=t0_rot_global_enu,
        )
        ego_future_xyz_t0_local[future_valid_mask] = valid_future_xyz_t0_local
        ego_future_rot_t0_local[future_valid_mask] = valid_future_rot_t0_local
    ...
    ego_future_valid_mask_tensor = (
        torch.from_numpy(future_valid_mask.astype(np.bool_)).unsqueeze(0).unsqueeze(0)
    )
    ...
    for camera_path in camera_paths:
        frames_tensor, clipped_indices, num_decoded_frames = _sample_camera_frames_with_fallback(
            camera_path,
    ...
    return {
        ...
        "ego_future_valid_mask": ego_future_valid_mask_tensor,
        "num_valid_future_steps": num_valid_future_steps,
        "future_truncated": future_truncated,
