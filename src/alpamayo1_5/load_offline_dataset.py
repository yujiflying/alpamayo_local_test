...
def _parse_gpfpd_line(line: str) -> tuple[float, float, float, float, np.ndarray] | None:
    ...
    rotation_raw = rotation.as_matrix()

    # The raw GPFPD-derived rotation behaves like:
    #   col1 = forward
    #   col0 = lateral
    #
    # But the model expects rotations whose yaw is extracted from col0:
    #   yaw = atan2(rot[..., 1, 0], rot[..., 0, 0])
    #
    # So remap the raw basis to a model-friendly basis:
    #   new0 = old1
    #   new1 = -old0
    #   new2 = old2
    #
    # This keeps a right-handed basis and makes col0 behave as forward.
    R_colmap = np.array(
        [
            [0.0, -1.0, 0.0],
            [1.0,  0.0, 0.0],
    ...
    rotation_model = rotation_raw @ R_colmap
    return gps_time_sod, latitude_deg, longitude_deg, altitude_m, rotation_model
...
def load_offline_dataset(
    ...
    # Step 2: apply a fixed +90 degree planar rotation to align the
    # offline local-frame xyz convention with the model/action-space convention.
    #
    # For xyz:
    #   x' = -y
    #   y' =  x
    #
    # Rotations are already remapped at parse time into a model-friendly
    # col0-forward convention, so we do not apply an additional rot fix here.
    R_fix = np.array(
        [
            [0.0, -1.0, 0.0],
            [1.0,  0.0, 0.0],
