...
FINAL unified-frame version:
- xyz and rotations are both returned in the same model/action frame
- notebooks should NOT apply an extra eval-side xy rotation anymore
...
2. t0-local ego frame
   - Origin at ego pose of t0
   - Raw local frame before model/action conversion

3. Model/action frame
   - Final unified frame returned by this loader
   - Both xyz and rotation tensors are expressed in this frame
   - Intended to be used directly by inference and plotting
"""
...
def _load_ego_pose_log(
    ...
    """Load ego poses from ego_pos.log using only $GPFPD lines."""
...
def _convert_global_enu_pose_to_t0_local_frame(
    ...
    """Convert global ENU poses into a raw t0-centered local ego frame."""
...
def _get_t0_local_to_model_action_rotation() -> np.ndarray:
    """Fixed planar transform from raw t0-local frame to model/action frame."""
    return np.array(
        [
            [0.0, -1.0, 0.0],
            [1.0,  0.0, 0.0],
...
def _convert_t0_local_xyz_to_model_action_frame(
    xyz_t0_local: np.ndarray,
) -> np.ndarray:
    """Convert raw t0-local xyz into the unified model/action frame."""
    R_t0_to_model = _get_t0_local_to_model_action_rotation()
    return np.einsum("ij,tj->ti", R_t0_to_model, xyz_t0_local)


def _convert_t0_local_rot_to_model_action_frame(
    rot_t0_local: np.ndarray,
) -> np.ndarray:
    """Convert raw t0-local rotations into the unified model/action frame."""
    R_t0_to_model = _get_t0_local_to_model_action_rotation()
    return np.einsum("ij,tjk->tik", R_t0_to_model, rot_t0_local)
...
def load_offline_dataset(
    ...
    FINAL unified behavior:
    - Returned xyz tensors are in model/action frame
    - Returned rotation tensors are in model/action frame
    - notebooks should use returned xyz directly for plotting / metrics
    """
    ...
    # FINAL unified frame: xyz and rotations are both converted to model/action frame.
    ego_history_xyz_model_frame = _convert_t0_local_xyz_to_model_action_frame(
        ego_history_xyz_t0_local
    )
    ego_future_xyz_model_frame = _convert_t0_local_xyz_to_model_action_frame(
        ego_future_xyz_t0_local
    )
    ...
    _debug_print(debug, f"history_last_model_xyz={ego_history_xyz_model_frame[-1].tolist()}")
    _debug_print(debug, f"future_first_model_xyz={ego_future_xyz_model_frame[0].tolist()}")
    _debug_print(debug, f"future_last_model_xyz={ego_future_xyz_model_frame[-1].tolist()}")

    ego_history_xyz_tensor = (
        torch.from_numpy(ego_history_xyz_model_frame.astype(np.float32)).unsqueeze(0).unsqueeze(0)
    )
    ...
    ego_future_xyz_tensor = (
        torch.from_numpy(ego_future_xyz_model_frame.astype(np.float32)).unsqueeze(0).unsqueeze(0)
    )
    ...
    _debug_print(
        debug,
        "note=FINAL unified frame: ego_history_xyz / ego_future_xyz / "
        "ego_history_rot / ego_future_rot are all in model/action frame",
    )
    ...
    return {
        ...
        "coordinate_frame_summary": {
            "position_frame": "model_action_frame",
            "rotation_frame": "model_action_frame",
            "global_frame": "global_enu",
            "note": (
                "FINAL unified frame: xyz and rotation tensors are both in "
                "model/action frame; notebook plotting and metrics should use "
                "returned xyz directly without extra eval-side xy rotation"
            ),
        },
    }