...
def load_offline_dataset(
    ...
    original_shape = tuple(frames_tensor.shape)
    frames_tensor = _resize_frames(frames_tensor, image_size)

    _debug_print(
        debug,
        f"camera={camera_path.name}, cam_idx={cam_idx}, "
        f"num_video_frames={num_decoded_frames}, "
        f"original_shape={original_shape}, "
        f"resized_shape={tuple(frames_tensor.shape)}, "
        f"actual_frame_indices={clipped_indices.tolist()}",
    )
