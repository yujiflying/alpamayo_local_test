...
def _load_ego_pose_log(
    ...
    Confirmed ``$GPFPD`` field semantics:
        0:  $GPFPD
        1:  GPS week
        2:  GPS time (seconds-of-day)
        3:  Heading (deg)          [already ENU yaw: east=0, CCW positive]
        4:  Pitch (deg)
        5:  Roll (deg)
        6:  Latitude (deg)
        7:  Longitude (deg)
        8:  Altitude (m)
        9:  Ve (east velocity, m/s)
        10: Vn (north velocity, m/s)
        11: Vu (up velocity, m/s)
        ... remaining fields ignored

    Important:
        - Heading in this log is already in the ENU yaw convention.
        - Therefore DO NOT apply a navigation-heading conversion such as:
              yaw = 90 - heading
        - Position is derived from lat/lon/alt and converted into a local ENU
          metric frame anchored at the first valid sample.

    Returns:
        timestamps_sod:
            Shape [N]. GPS seconds-of-day.
        xyz_enu:
            Shape [N, 3]. Local ENU coordinates in meters.
        rotations:
            ``scipy.spatial.transform.Rotation`` of length N.
    """
    ...
    with ego_log_path.open("r") as f:
        for line_idx, raw_line in enumerate(f, start=1):
            ...
            try:
                gps_week = float(parts[1])   # parsed for completeness / debugging
                sod = float(parts[2])
                ...
                alt_m = float(parts[8])
                ve_mps = float(parts[9])     # parsed for completeness / debugging
                vn_mps = float(parts[10])    # parsed for completeness / debugging
                vu_mps = float(parts[11])    # parsed for completeness / debugging
    ...
    # IMPORTANT:
    # Heading in ego_pos.log is already ENU yaw with:
    #   east = 0 deg, counterclockwise positive
    # Therefore do not apply any extra heading-to-yaw conversion.
    yaw_deg = heading_deg
...
def load_offline_dataset(
    ...
    Important implementation notes:
        1. Global pose is first represented in ENU.
        2. Poses are converted into a t0-centered local frame.
        3. A fixed planar transform is applied to rotations only, not xyz.
           This combination has been empirically validated:
             - history dxy / yaw consistency becomes near-perfect
             - long-horizon speed collapse disappears
        4. The remaining ~90 degree mismatch observed in offline evaluation is an
           evaluation-frame issue and should be handled on the notebook / metric
           side, not by changing model-input history geometry here.
    """
    ...
    # Step 1:
    # Convert global ENU poses into a t0-centered local frame.
    ...
    # Step 2:
    # Align the local rotation convention with the model/action-space convention.
    #
    # IMPORTANT:
    # - Do NOT apply the fixed planar rotation to xyz.
    # - Apply the fixed transform to rotations only.
    # - This exact combination has been empirically validated:
    #     * history dxy / yaw consistency becomes near-perfect
    #     * long-horizon speed collapse disappears
    # - The remaining ~90deg mismatch observed offline is an evaluation-side
    #   frame issue and should be corrected when plotting / scoring GT.
    R_fix = np.array(
        [
            [0.0, -1.0, 0.0],
            [1.0,  0.0, 0.0],
    ...
    _debug_print(
        debug,
        "note=offline GT/history visualization may require an eval-side -90deg xy rotation "
        "to compare against model outputs in +x-forward convention",
    )
