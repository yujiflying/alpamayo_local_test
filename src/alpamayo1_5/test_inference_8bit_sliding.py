...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-jsonl",
        default="sliding_inference_results.jsonl",
        help="Where to save per-window results",
    )
    ...
    with output_path.open("w", encoding="utf-8") as f:
        for idx, t0_sod in enumerate(t0_values, start=1):
            ...
            result = {
                ...
                "image_frame_indices": data["video_frame_indices"][0].cpu().tolist(),
                "camera_indices": data["camera_indices"].cpu().tolist(),
                "cot": extra["cot"][0].tolist() if hasattr(extra["cot"][0], "tolist") else extra["cot"][0],
                "min_ade": min_ade,
            }
            f.write(json.dumps(result, ensure_ascii=False) + "\n")
            f.flush()

            print(f"  frame_index_t0={frame_index_t0}, minADE={min_ade:.3f}")
