...
OFFLINE_CAMERA_INDEX = {
    ...
    "rear_camera": 4,
    "rear_right_camera": 5,
    "front_telephoto_camera": 6,
}

OFFLINE_CAMERA_FILES = {
    "Front_left_camera.mp4": 0,
    "Front_camera.mp4": 1,
    "Front_right_camera.mp4": 2,
    "Front_telephoto_camera.mp4": 6,
}


def resolve_processor_name(processor_name_or_path: str | Path | None = None) -> str:
    """Resolve the processor identifier/path to use for AutoProcessor loading."""
    if processor_name_or_path is None:
        return BASE_PROCESSOR_NAME
    return str(Path(processor_name_or_path).expanduser())


def infer_camera_index(camera_name_or_path: str | Path) -> int:
    """Infer the training camera index from an offline camera filename/path."""
    path = Path(camera_name_or_path)
    normalized = path.stem.lower().replace("-", "_").replace(" ", "_")
    if normalized in OFFLINE_CAMERA_INDEX:
        return OFFLINE_CAMERA_INDEX[normalized]
    raise KeyError(f"Unsupported offline camera name: {camera_name_or_path}")


def discover_offline_camera_files(clip_dir: str | Path) -> list[Path]:
    """Return offline camera video files in training camera order."""
    clip_dir = Path(clip_dir).expanduser()
    found_paths = []
    for filename, _ in sorted(OFFLINE_CAMERA_FILES.items(), key=lambda item: item[1]):
        path = clip_dir / filename
        if path.exists():
            found_paths.append(path)
    if not found_paths:
        raise FileNotFoundError(
            f"No supported offline camera videos found under {clip_dir}. "
            f"Expected one or more of: {', '.join(OFFLINE_CAMERA_FILES)}"
        )
    return found_paths
...
def get_processor(
    ...
    """Get the processor for the Qwen VL model used by Alpamayo."""
    processor_kwargs = {
    ...
    processor = AutoProcessor.from_pretrained(
        resolve_processor_name(processor_name_or_path),
        **processor_kwargs,
    )
